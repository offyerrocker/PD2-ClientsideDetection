...
--function SecurityCamera:sync_set_detection_enabled(state,str_settings,_)
...
Hooks:PostHook(SecurityCamera,"set_detection_enabled","clientsidedetection_setcameraupdateenabled",function(self,state,settings,mission_element)
	...
	-- store the last "detection enabled" state 
	-- to sync to any dropins
	if Network:is_server() then -- should be implicit after the 64 bit update
		self._cd_detection_state = {
			state = state,
			settings = settings
		}
		local str_settings = (settings and json.encode(settings)) or "null"
		managers.network:session():send_to_peers_synched("sync_set_camera_detection_enabled", self._unit, state, str_settings)
	end
end)
